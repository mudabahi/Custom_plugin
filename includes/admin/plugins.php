 ...
 * @copyright   Copyright (c) 2023, Best Donation
 * @license     There is a Best Donation License
 * @since       1.4
 */
...
function give_in_plugin_update_message( $data, $response ) {
	...
	// Get the upgrade notice from the trunk.Add-ons
...
function give_deactivation_popup() {
	...
	<h2 id="deactivation-survey-title">
		...
		<span><?php esc_html_e( 'Best Donation Here', 'give' ); ?></span>
	</h2>
	<form class="deactivation-survey-form" method="POST">
		<p><?php esc_html_e( 'If you have a moment, please let us know why you are deactivating Best Donation. All submissions are anonymous and we only use this feedback to improve this plugin.', 'give' ); ?></p>

		<div>
			<label class="give-field-description">
				<input type="radio" name="give-survey-radios" value="2">
				<?php esc_html_e( 'I am using Best Donation', 'give' ); ?>
			</label>
		</div>
		...
		<div id="survey-and-delete-data">
			<p>
				<label>
					...
					<?php esc_html_e( 'Would you like to delete all Best Donation data?', 'give' ); ?>
				</label>
				<section class="give-field-description">
					<?php esc_html_e( 'Best Donation  here.', 'give' ); ?>
				</section>
			</p>
		</div>
...
function give_deactivation_form_submit() {
    ...
    wp_remote_post(
        // 'http://survey.#.com/wp-json/give/v2/survey/',
        [
            'body' => [
