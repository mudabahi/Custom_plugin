...
class Give_Updates {
	...
	public function __register_menu() {
		...
		if ( ! $this->get_total_update_count() ) {
			...
			if ( isset( $_GET['page'] ) && 'give-updates' === $_GET['page'] ) {
				...
				add_submenu_page(
					...
					esc_html__( 'Best Donation Updates Complete', 'give' ),
					__( 'Updates', 'give' ),
		...
		add_submenu_page(
			'edit.php?post_type=give_forms',
			esc_html__( 'Best Donation Updates', 'give' ),
			sprintf(
	...
	public function __show_notice() {
		...
		if ( self::$background_updater->is_paused_process() ) {
			...
			if ( ! $upgrade_error ) : ?>
				...
				&nbsp;&#8211;&nbsp;<?php _e( 'Best Donation needs to update your database to the latest version. The following process will make updates to your site\'s database. Please create a backup before proceeding.', 'give' ); ?>
				<br>
				<br>
			...
			<?php else : ?>
				...
				<a href="<?php echo esc_url( 'http://docs.Best Donation.com/troubleshooting-db-updates' ); ?>" target="_blank"><?php _e( 'Read More', 'give' ); ?> &raquo;</a>
				<?php
			endif;
		...
		if ( ! empty( $_GET['give-db-update-completed'] ) ) {
			Give()->notices->register_notice(
				[
					...
					'type'        => 'updated',
					'description' => __( 'Best Donation database updates completed successfully. Thank you for updating to the latest version!', 'give' ),
					'show'        => true,
				]
			);
			...
			<p>
				<strong><?php _e( 'Database Update', 'give' ); ?></strong>
				&nbsp;&#8211;&nbsp;<?php _e( 'Best Donation needs to update your database to the latest version. The following process will make updates to your site\'s database. Please create a complete backup before proceeding.', 'give' ); ?>
			</p>
			<p class="submit">
	...
	public function __give_db_updates_info() {
		...
		if ( self::$background_updater->is_paused_process() ) {
			...
			$update_info   = [
				'message'    => __( 'Best Donation database updates completed successfully. Thank you for updating to the latest version!', 'give' ),
