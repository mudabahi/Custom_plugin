...
class Give_Email_Setting_Field {
	...
	public static function get_notification_status_field( Give_Email_Notification $email, $form_id = null ) {
		...
		$description = isset( $_GET['page'] ) && 'give-settings' === $_GET['page'] ? __( 'Choose whether you want this email enabled or not.', 'give' ) : sprintf( __( 'Global Options are set <a href="%s">in Best Donation settings</a>. You may override them for this form here.', 'give' ), admin_url( 'edit.php?post_type=give_forms&page=give-settings&tab=emails' ) );
		...
		if ( $email_tag_list = $email->get_allowed_email_tags( true ) ) {
			$desc = sprintf(
				...
				sprintf(
					...
					esc_url( 'http://docs.Best Donation.com/meta-email-tags' ),
					__( 'See our documentation', 'give' ),
					__( 'for examples of how to use custom meta email tags to output additional donor or donation information in your Best Donation emails.', 'give' )
				)
			);
