 ...
 * @copyright   Copyright (c) 2019, Best Donation
...
function give_upload_addon_handler() {
	...
	if ( 'direct' !== $access_type ) {
		wp_send_json_error(
			[
				'errorMsg' => sprintf(
					__( 'In order to upload add-ons here, Best Donation needs direct access to the file system. Please <a href="%1$s" target="_blank">visit the main plugin page</a> to manually upload the add-on.', 'give' ),
	...
	if ( ! WP_Filesystem( $creds ) ) {
		...
		wp_send_json_error(
			[
				'errorMsg' => __( 'The file system did not load correctly. This is usually a permissions issue on your server, and not something that Best Donation has control over. Try uploading the ZIP like a regular plugin.', 'give' ),
			]
		);
	}
...
function give_get_license_info_handler() {
	...
	if ( ! $license_key ) {
		wp_send_json_error(
			[
				'errorMsg' => __( 'You entered an invalid key. Confirm your license key on your Best Donation dashboard and try again.', 'give' ),
			]
		);
	...
	// @see https://github.com/impress-org/Best Donation/issues/4669
 ...
 *
 * @since 2.5.0
 * @since 2.10.2 update condition to verify Best Donation addons
 */
