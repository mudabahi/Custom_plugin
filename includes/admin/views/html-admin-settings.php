...
if ( ! empty( $tabs ) && array_key_exists( give_get_current_setting_tab(), $tabs ) ) :
        ...
        */
        ...
        <div class="nav-tab-wrapper give-nav-tab-wrapper">
				...
				if ( $isReports === true ) {
					echo sprintf(
						...
						esc_html__( 'Neues Berichts-Dashboard', 'give' )
					);
				}
				?>

				<div class="give-sub-nav-tab-wrapper">
					<a href="#" id="give-show-sub-nav" class="nav-tab give-not-tab" title="<?php esc_html_e( 'Verbleibende Einstellungsregisterkarten anzeigen', 'give' ); ?>">
		...
		if ( empty( $GLOBALS['give_hide_save_button'] ) ) :
			...
			<div class="give-submit-wrap">
				<input name="save" class="button-primary give-save-button" type="submit" value="<?php esc_html_e( 'Änderungen speichern', 'give' ); ?>"/>
			</div>
		<?php endif; ?>
	...
	<div class="error">
		<p>
			<?php esc_html_e( 'Hoppla, diese Einstellungsseite existiert nicht.', 'give' ); ?>
		</p>
	</div>
	<?php
endif;
