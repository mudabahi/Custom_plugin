...
function give_api_callback() {
	...
	<span class="give-metabox-description api-description">
		...
		echo sprintf(
			...
			__( 'Sie können API-Schlüssel für einzelne Benutzer in ihrem Profilbearbeitungsbildschirm erstellen. Mit API-Schlüsseln können Benutzer die <a href="%1$s" target="_blank">GiveWP REST API</a> verwenden, um Spendendaten in JSON oder XML für externe Anwendungen oder Geräte wie <a href= abzurufen „%2$s“ target="_blank">Zapier</a>.', 'give' ),
			esc_url( 'http://docs.#.com/api' ),
			esc_url( 'http://docs.#.com/addon-zapier' )
		);
		?>
	</span>
	<?php
